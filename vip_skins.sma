// *************************************************************************************//
// Плагин загружен с  www.neugomon.ru                                                   //
// Автор: Neygomon                                                                      //
// При копировании материала ссылка на сайт www.neugomon.ru ОБЯЗАТЕЛЬНА!                //
// *************************************************************************************//

#include <amxmodx>
#include <saytext>
#include <hamsandwich>
enum _:DATA { Name[64], Cmd[32], Flags }

#define ADMIN_LOADER        // Выводить срок до конца админки с Admin Loader by Neugomon
new g_szDefaultItems[][DATA] =    // Стандартные пункты в меню | { "название пункта", "команда", "флаг доступа" }
{
	{ "\r[\yВзять | Дигл\r]",            "say /dg1",      ADMIN_LEVEL_H },
    { "\r[\yВзять | AK47\r]",            "say /ak1",         ADMIN_LEVEL_H },
    { "\r[\yВзять | M4A\r]",             "say /m41",     ADMIN_LEVEL_H },
	{ "\r[\yВзять | Famas\r]",             "say /fm1",     ADMIN_LEVEL_H },
    { "\r[\yВзять | AWP\r]",             "say /aw1",     ADMIN_LEVEL_H }
}

new g_iNumItems;
new g_szMenuData[128][DATA];
new g_iMenuPage[33];
new g_iChosenMenu[33];
new g_iRoundCount;
#if defined ADMIN_LOADER
	native admin_expired(index);
#endif	
public plugin_init()
{
	register_plugin("Menus Front-End", "1.5", "neugomon");
	
	register_clcmd("say /vip", 		"clcmdAdminMenu", ADMIN_LEVEL_H);
	register_srvcmd("amx_addmenuitem", 	"SrvAddMenuItems");
	register_event("HLTV", "Event_HLTV", "a", "1=0", "2=0");
	RegisterHam(Ham_Spawn, "player", "Ham_PlayerSpawn_Post", true);
	register_logevent("LogEvent_RestartGame", 2, "1=Game_Commencing", "1&Restart_Round_");
	register_clcmd("say /choosemenu", "Show_ChooseStartMenu");

	register_menucmd(register_menuid("Show_ChooseStartMenu"), 1023, "Handle_ChooseStartMenu");
	register_menucmd(register_menuid("AMX Menu"), 1023, "amxx_handler");
	
	MenuDefaultLoad();
}

public client_connect(id)
{
	g_iChosenMenu[id] = -1;
}

public LogEvent_RestartGame()
{
	g_iRoundCount = 0;
}

public Ham_PlayerSpawn_Post(id)
{
	if(is_user_alive(id) && get_user_flags(id) & ADMIN_LEVEL_H)
	{
		switch(g_iChosenMenu[id])
		{
			case -1:
			{
				Show_ChooseStartMenu(id);
			}
			case 0:
			{
				if(g_iRoundCount > 2)
					client_cmd(id, "say /vip");
			}
			case 1:
			{
				if(g_iRoundCount > 2)
					client_cmd(id, "say /vipmenu");
			}
		}
	}
}

public Event_HLTV()
{
	g_iRoundCount++;
}

public Show_ChooseStartMenu(id)
{
	if(!(get_user_flags(id) & ADMIN_LEVEL_H)) return PLUGIN_HANDLED;
	new szMenu[512], iKeys = (1<<0|1<<1|1<<2|1<<9), iLen = formatex(szMenu, charsmax(szMenu), "\wМеню при возрождении:^n^n");
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\r[\y1\r] \wМеню скинов^n");
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\r[\y2\r] \wМеню оружий^n");
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\r[\y3\r] \wНе показывать^n");
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "^n\r[\y0\r] \wВыход");

	return show_menu(id, iKeys, szMenu, -1, "Show_ChooseStartMenu");
}

public Handle_ChooseStartMenu(id, iKey)
{
	switch(iKey)
	{
		case 9:
		{
			return PLUGIN_HANDLED;
		}
		default:
		{
			g_iChosenMenu[id] = iKey;
			if(iKey == 2)
			{
				UTIL_SayText(id, "!gSERVER !t| !yВы отключили показ меню в начале раунда."); 
				UTIL_SayText(id, "!gSERVER !t| !yВключить его снова можно в главном меню.");
				return PLUGIN_HANDLED;
			}
			UTIL_SayText(id, "!gSERVER !t| !yВы выбрали !g%s!y.", (iKey == 0 ? "меню скинов" : "меню оружий"));
			UTIL_SayText(id, "!gSERVER !t| !yМеню начнет появляться с третьего раунда.");
		}
	}
	return PLUGIN_HANDLED;
}

public clcmdAdminMenu(id, flags)
{
	if(get_user_flags(id) & flags)
	{
		g_iMenuPage[id] = 0;
		BuildMenu(id, g_iMenuPage[id]);
	}	
	else	console_print(id, "* Вы не имеете доступа к этой команде");
	return PLUGIN_HANDLED;
}

public SrvAddMenuItems()
{
	if(read_argc() >= 3)
	{
		new ItemName[64], ItemCmd[32], ItemFlags[30];
		read_argv(1, ItemName, charsmax(ItemName));
		read_argv(2, ItemCmd, charsmax(ItemCmd));
		read_argv(3, ItemFlags, charsmax(ItemFlags));
		
		copy(g_szMenuData[g_iNumItems][Name], 	charsmax(g_szMenuData[][Name]), ItemName);
		copy(g_szMenuData[g_iNumItems][Cmd], 	charsmax(g_szMenuData[][Cmd]), 	ItemCmd);
		g_szMenuData[g_iNumItems][Flags] = 	read_flags(ItemFlags);
		g_iNumItems++;
	}
	else	server_print("[Menus Front-End by Neugomon] Item not added! Syntax: ^"Item name^" ^"Item command^" ^"Item access^"");
	return PLUGIN_HANDLED;
}

MenuDefaultLoad()
{
	for(new i; i < sizeof g_szDefaultItems; i++)
	{
		copy(g_szMenuData[g_iNumItems][Name], 	charsmax(g_szMenuData[][Name]), g_szDefaultItems[i][Name]);
		copy(g_szMenuData[g_iNumItems][Cmd],  	charsmax(g_szMenuData[][Cmd]),  g_szDefaultItems[i][Cmd]);
		g_szMenuData[g_iNumItems][Flags] = 	g_szDefaultItems[i][Flags];
		g_iNumItems++;
	}
}

BuildMenu(id, pos)
{
	new szMenu[512];
	new len
	new start 	= pos * 8;
	new end 	= start + 8;
	new keys	= MENU_KEY_0;
	
	if(start >= g_iNumItems)
		start = pos = g_iMenuPage[id] = 0;
	if(g_iNumItems == 9 || end > g_iNumItems)
		end = g_iNumItems;	
#if defined ADMIN_LOADER
	new exp = admin_expired(id);
	if(exp > 0)
	{
		new systime = get_systime();
		if(exp - systime > 0)
		{
			if((exp - systime) / 86400 > 0)
				len = formatex(szMenu, charsmax(szMenu), "\rСкины^n\wОсталось \r%d \wдней^n^n", ((exp - systime) / 86400));
			else	len = formatex(szMenu, charsmax(szMenu), "\rСкины^n\rПоследний \wдень^n^n");
		}
	}
	else if(exp == 0)	len = formatex(szMenu, charsmax(szMenu), "\rСкины^n\wПрава \yне ограничены^n^n");
#else
	len = formatex(szMenu, charsmax(szMenu), "\rСкины^n\wВыберите действие^n^n");
#endif
	for(new i = start, flags = get_user_flags(id), a; i < end; i++)
	{
		if(flags & g_szMenuData[i][Flags])
		{
			keys |= (1 << a);
			len += formatex(szMenu[len], charsmax(szMenu) - len, "\r%d. \w%s^n", ++a, g_szMenuData[i][Name]);
		}
		else	len += formatex(szMenu[len], charsmax(szMenu) - len, "\r%d. \d%s^n", ++a, g_szMenuData[i][Name]);
	}	
	
	if(end != g_iNumItems)
	{
		formatex(szMenu[len], charsmax(szMenu) - len, "^n\r9. \yДалее^n\r0. \r%s", pos ? "Назад" : "Выход");
		keys |= MENU_KEY_9;
	}
	else formatex(szMenu[len], charsmax(szMenu) - len, "^n\r0. \r%s", pos ? "Назад" : "Выход");
	
	return show_menu(id, keys, szMenu, -1, "AMX Menu");
}

public amxx_handler(id, key)
{
	switch(key)
	{
		case 9: if(g_iMenuPage[id]) BuildMenu(id, --g_iMenuPage[id]);
		default:
		{
			if(key == 8 && g_iNumItems > 9)
				BuildMenu(id, ++g_iMenuPage[id]);
			else
			{
				new pos = g_iMenuPage[id] * 8 + key;
				if(g_szMenuData[pos][Cmd][0] == 's' && g_szMenuData[pos][Cmd][1] == 'v' && g_szMenuData[pos][Cmd][2] == '_')
					server_cmd(g_szMenuData[pos][Cmd]);
				else 	client_cmd(id, g_szMenuData[pos][Cmd]);
			}
		}
	}	
	return PLUGIN_HANDLED;
}