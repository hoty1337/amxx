...
native jbe_chat_prefix(str[], len);
...
#define COLOR_BUFFER 8
#define FILE_PREFIXES "jb_engine/chat_prefix.ini"
...
public plugin_init()
{
	register_plugin("[JBE] LTranslit", "4.0", "neygomon && LeGioN");
...
public client_disconnect(id) ClearBit(g_iBitIgnorePlayer, id), ClearBit(g_iBitTranslate, id);
public LangRus(id)
{
	new quests_str[64];
	jbe_chat_prefix(quests_str, charsmax(quests_str));
	//if(is_user_steam(id)) return;
	if(!IsSetBit(g_iBitTranslate, id)) 
	{
		SetBit(g_iBitTranslate, id);
		ChatColor(id,"%s !yВы успешно активировали !tрусский язык!y.", quests_str);
		client_cmd(id, "spk buttons/blip1.wav");
	} 
	else ChatColor(id,"%s !yПростите, Вы уже активировали русский !tязык!y.", quests_str);
}
public LangEng(id)
{
	new quests_str[64];
	...
	if(IsSetBit(g_iBitTranslate, id))
	{
		...
		ChatColor(id,"%s !yВы успешно активировали !tанглийский язык!y.", quests_str);
		client_cmd(id, "spk buttons/blip1.wav");
	} 
	else ChatColor(id,"%s !yПростите, Вы уже активировали английский !tязык!y.", quests_str);
}
...
public FormatMsg(id, bool:is_say_team)
{
	...
	new idTeam = jbe_get_user_team(id);
	switch(jbe_get_user_team(id))
	{
		case 1: iLen = formatex(szFormatTags, charsmax(szFormatTags), "^3[>] %s%s", is_user_alive(id) ? "^1" : "^1**^3Мёртв^1** ", is_say_team ? "^1[^3Заключенный^1] " : "^1[^3Заключенный^1] ");
		case 2: iLen = formatex(szFormatTags, charsmax(szFormatTags), "^3[>] %s%s", is_user_alive(id) ? "^1" : "^1**^3Мёртв^1** ", is_say_team ? "^1[^3Охранник^1] " : "^1[^3Охранник^1] ");
		default: iLen = formatex(szFormatTags, charsmax(szFormatTags), "^1**^3Зритель^1** ");
	}
	if(g_iArrayFlagSize){
	...
	iLen += formatex(szFormatTags[iLen], charsmax(szFormatTags) - iLen, "^3%s^1: %s", szName, get_user_flags(id) & ADMIN_LEVEL_D ? "^4" : "");
#else
	iLen += formatex(szFormatTags[iLen], charsmax(szFormatTags) - iLen, "^3%s^1: ", szName);
#endif
	read_args(szMessage, charsmax(szMessage));
	remove_quotes(szMessage); trim(szMessage);
	if(!szMessage[0] || szMessage[0] == EOS) return PLUGIN_HANDLED;  // блочим пустую мессаг
	if(IsSetBit(g_iBitTranslate, id))
	{
	...
	while(iLen + strlen(szMsg) > 160) szMsg[iByteLimit -= 10] = 0;
	iLen += formatex(szFormatTags[iLen], charsmax(szFormatTags) - iLen, "%s", szMsg);
	
#if defined FIX_CRASH
	if(strlen(szFormatTags) + strlen(szMsg) >= 250)
	{
		ChatColor(id,"%s !yПростите, но ваше !tсообщение !yсдишком длинное!", quests_str);
	...
	switch(is_say_team){
		...
		case 1:{
			...
			for(new i; i < pcount; i++){
				if(jbe_get_user_team(id) == jbe_get_user_team(players[i]))
				{	
					SendMsgChat(players[i], idTeam, szFormatTags);
					new szCurrentTime[32];
					get_time("%X", szCurrentTime, charsmax(szCurrentTime));
