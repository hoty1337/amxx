...
public plugin_init()
{
	...
	register_clcmd("amx_rtv", "Command_FastRtv");
...
public Command_FastRtv(id)
{
	if(!(get_user_flags(id) & ADMIN_BAN)) return PLUGIN_HANDLED;
	StartVote(0);
	client_print_color(0, print_team_default, "%s^1 Администратор ^4%n ^1запустил досрочную смену карты.", PREFIX, id);
	return PLUGIN_CONTINUE;
}
