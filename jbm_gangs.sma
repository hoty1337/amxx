...
/* Constants */
	...
	new const g_szPrefix[ ] = "^04[БАНДЫ]^01";
...
public plugin_init()
{
	...
	g_pUpdateCost			= register_cvar( "jb_update_cost", 		"10" );
	g_pCreateCost			= register_cvar( "jb_gang_cost", 		"1" );
	...
	g_pFee					= register_cvar( "jb_fee", 	"10" );
	g_pCommonFundMax		= register_cvar( "jb_commonfund_max", 	"10" );
	...
	g_pStaminaPerLevel		= register_cvar( "jb_stamina_per", 		"4" 	);
	g_pWeaponDropPerLevel 	= register_cvar( "jb_weapondrop_per", 	"1" 	);

	g_pPointsPerKill		= register_cvar( "jb_points_per_kill",	"0" );
	g_pHeadshotBonus		= register_cvar( "jb_headshot_bonus",	"0" );
	...
	g_pPrefixGang			= register_cvar( "jb_gang_prefix_status", 	"0" ); // Admins can create gangs without points
...
public client_disconnected( id )
{
	...
	if(g_iSql)
	{
		...
		formatex(query,charsmax(query), "UPDATE `%s` \
				...
				g_eCacheCvars[iDBPrefix_Gang_Points], g_iPoints[ id ], get_systime(), szAuthID);
...
public client_putinserver( id )
{
	...
	if(g_iSql)
	{
		...
		formatex(query,charsmax(query),  "SELECT * FROM %s WHERE `key` = '%s'", g_eCacheCvars[iDBPrefix_Gang_Points], szAuthID);
...
{
	register_native("gang_set_points", "gang_set_points", 1);
    register_native("gangname", "native_gangname")
}
public native_gangname()
{
	if(g_iGang[get_param(3)] == -1 ) return PLUGIN_HANDLED;
	new aData[ GangInfo ];
	ArrayGetArray( g_aGangs, g_iGang[ get_param(3) ], aData );
	
	new dlina = get_param(2);
    set_string(1, aData[GangName], dlina);
    return 1;
}
public gang_set_points(id, iCount)
{
	g_iPoints[id] = iCount;
}
...
public plugin_end()
{
	...
	for( new i = 0; i < ArraySize( g_aGangs ); i++ )
	{
		...
		formatex(query,charsmax(query),"UPDATE `%s` SET `HP` = '%d', `Stealing` = '%d',	`Gravity` = '%d', `Stamina` = '%d', `WeaponDrop` = '%d', `Damage` = '%d', `Kills` = '%d', `Money` = '%d', `MoneyLevel` = '%d'\
		WHERE `GangName` = '%s';", 
...
{	
	...
	if( g_iGang[ id ] > -1 )
	{
		...
		FormatItem("\r[1] \dСоздать Банду \r[\y$%i\r]^n", get_pcvar_num( g_pCreateCost ) );
	}
	
	else
	{
		FormatMain("\yМеню банды^n\wТекущая банда:\r Нету^n" );
		FormatItem("\r[1] \wСоздать Банду \r[\y$%i\r]^n", get_pcvar_num( g_pCreateCost ) );
	...
	{
		FormatItem("\r[2] \wПригласить игрока^n" );
		iKeys |= MENU_KEY_2;
	}
	else
		FormatItem("\r[2] \dПригласить игрока^n" );
	
	if( g_iGang[ id ] > -1 )
	{
		FormatItem("\r[3] \wСкиллы^n" );
		iKeys |= MENU_KEY_3;
	}
	
	else
		FormatItem("\r[3] \dСкиллы^n" );
		
	FormatItem("\r[4] \wTop-10^n" );
	
	if( g_iGang[ id ] > -1 )
	{
		FormatItem("\r[5] \wВыйти с банды^n" );
		iKeys |= MENU_KEY_5;
	}
	
	else
		FormatItem("\r[5] \dВыйти с банды^n" );
	
	
	if( iStatus > STATUS_MEMBER )
	{
		FormatItem("\r[6] \wУправление бандой^n" );
		iKeys |= MENU_KEY_6;
	}
	
	else
		FormatItem("\r[6] \dУправление бандой^n" );
	
	if( g_iGang[ id ] > -1 )
	{
		FormatItem("\r[7] \wОнлайн игроки банды^n" );
		iKeys |= MENU_KEY_7;
	}
		
	else
		FormatItem("\r[7] \dОнлайн игроки банды^n" );

	if(g_iGang[id] > -1)
	{
		FormatItem("\r[8] \wОбщак^n" );
		iKeys |= MENU_KEY_8;
	}
	else
	{
		FormatItem("\r[8] \dОбщак^n" );
	}

	FormatItem("^n\r[0] \wВыход" );
...
public Show_CommonFund(id)
{
	...
	new aData[ GangInfo ];
	ArrayGetArray( g_aGangs, g_iGang[ id ], aData );
	new szMenu[512], iKeys = (1<<9), iLen = formatex(szMenu, charsmax(szMenu), "\wМеню общака^n\dВсего в общаке: $%d/$%d^n^n", aData[GangMoney], aData[GangMoneyLevel] * 100000);
...
public ClCmd_MoneyTransfer(id, isSend, iMoney)
{
	...
	if(isSend)
	{
		...
		else if (iMoney <= 0 || aData[GangMoneyLevel] * 100000 < aData[GangMoney] + iMoney) UTIL_SayText(id, "!g[JBM] !yНеверная сумма.");
		else
		{
	...
	{
		if (aData[GangMoney] < iMoney) UTIL_SayText(id, "!g[JBM] !yНедостаточно средств.");
...
public Cmd_CreateGang( id )
{
	...
	if( !bAdmin )
	{
		jbm_set_user_money(id, jbm_get_user_money(id) - get_pcvar_num( g_pCreateCost ), 1);
	}
	...
	formatex(query,charsmax(query), 
		"INSERT INTO %s \
		(`GangName`, `HP`, `Stealing`, `Gravity`, `Stamina`, `WeaponDrop`, `Damage`, `Kills`, `Money`, `MoneyLevel`) \
		VALUES ('%s', '0', '0', '0', '0', '0', '0', '0', '0', '1')",
...
public Cmd_ChangeNameGang( id )
{
	...
	{
		client_print_color( id, 0, "%s Вы не лидер для выполнения этого функции.", g_szPrefix );
		return PLUGIN_HANDLED;
	}
	
	new iRemaining = jbm_get_user_money(id) - get_pcvar_num( g_pChangeGangName );
...
public InviteConfirmMenu_Handler( id, hMenu, iItem )
{
	...
	{
		client_print_color( id, 0, "%s Вы не сможете выйти с банды, т.к. вы лидер.", g_szPrefix );
		return PLUGIN_HANDLED;
	}
...
{	
	static szMenu[ 512 ], iLen, iKeys = (1<<9), aData[ GangInfo ];	
	ArrayGetArray( g_aGangs, g_iGang[ id ], aData );


	iLen = formatex(szMenu, charsmax(szMenu), "\yПрокачка навыков^n\dВсего в общаке: $%d/$%d^n^n", aData[GangMoney], aData[GangMoneyLevel] * 100000);
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\r[1] \%sЗдоровье [\rЦена: \y$%i\w] \y[Level:%i/%i]^n", (get_pcvar_num( g_pUpdateCost ) <= aData[GangMoney] ? "w" : "d"), get_pcvar_num( g_pUpdateCost ), aData[ GangHP ], get_pcvar_num( g_pHealthMax ) );
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\r[2] \%sГравитация [\rЦена: \y$%i\w] \y[Level:%i/%i]^n", (get_pcvar_num( g_pUpdateCost ) <= aData[GangMoney] ? "w" : "d"), get_pcvar_num( g_pUpdateCost ), aData[ GangGravity ], get_pcvar_num( g_pGravityMax ) );
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\r[3] \%sПрибавка урона [\rЦена: \y$%i\w] \y[Level:%i/%i]^n", (get_pcvar_num( g_pUpdateCost ) <= aData[GangMoney] ? "w" : "d"), get_pcvar_num( g_pUpdateCost ), aData[ GangDamage ], get_pcvar_num( g_pDamageMax ) );
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\r[4] \%sСкорость [\rЦена: \y$%i\w] \y[Level:%i/%i]^n", (get_pcvar_num( g_pUpdateCost ) <= aData[GangMoney] ? "w" : "d"), get_pcvar_num( g_pUpdateCost ), aData[ GangStamina ], get_pcvar_num( g_pStaminaMax ) );
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\r[5] \%sРасширение общака [\rЦена: \y$%i\w] \y[Level:%i/%i]^n", (get_pcvar_num( g_pUpdateCost ) <= aData[GangMoney] ? "w" : "d"), get_pcvar_num( g_pUpdateCost ), aData[ GangMoneyLevel ], get_pcvar_num( g_pCommonFundMax ) );
	if(get_pcvar_num( g_pUpdateCost ) <= aData[GangMoney])
	{
		iKeys |= (1<<0|1<<1|1<<2|1<<3|1<<4);
	}
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "^n^n\r[0] \wВыход");
...
public SkillsMenu_Handler( id, iKey )
{
	...
	{		
		case 0:
		{
			...
			new iRemaining = aData[GangMoney] - get_pcvar_num( g_pUpdateCost );
		...
		/*case 2:
		{
			...
			jbm_set_user_money(id, iRemaining, 1);
		}*/
		...
		case 4:
		{
			if( aData[ GangMoneyLevel ] == get_pcvar_num( g_pCommonFundMax ) )
			{
				client_print_color( id, 0, "%s Ваша банда уже на максимальном уровне для этого навыка.", g_szPrefix  );
				ShowSkillsMenu( id );
				return PLUGIN_HANDLED;
...
public TransferMenu_Handler( id, hMenu, iItem )
{
	...
	if( !is_user_connected( iPlayer ) )
	{
		client_print_color( id, 0, "%s Этот игрок больше не в сети", g_szPrefix );
		ShowTransferMenu( id );
		return PLUGIN_HANDLED;
	}
