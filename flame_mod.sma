...
Создатель [VK]: vk.com/zefer_pro
Создатель Skype: gerasimov.ru5

Плагина на заказ [VK]: vk.com/amx_market
...
native GetShop(iPlayer);
native sp_get_toy(iPlayer);
...
#define CLIENT_CONNECT_SOUND	"sound/Flame_Md/remix_dip.mp3"
#define MENU_OFF "BuB9neC/off.wav"

// Plugin Version
new const PLUGIN_VERSION[] = "4.3 Fix4"
...
// Hack to be able to use Ham_Player_ResetMaxSpeed (by joaquimandrade) 0.87
...
new const WEAPONNAMES[][] = { "", "P228 Compact", "", "Schmidt Scout", "", "XM1014 M4", "", "Ingram MAC-10", "Steyr AUG A1",
			...
			"Взять \r[\yUSP\r]", "Взять \r[\yGlock\r]", "AWP Magnum Sniper", "MP5 Navy", "M249 Para Machinegun",
			"M3 Super 90", "Взять \r[\yM4A1\r]", "Schmidt TMP", "G3SG1 Auto-Sniper", "", "Взять \r[\yDeagle\r]",
			"SG-552 Commando", "Взять \r[\yAK-47\r]", "", "ES P90" }
...
Array:model_grand_human, Array:model_delux_human, Array:model_cr_human, Array:model_bog_human,
Array:model_coldun_human, Array:model_creator_human,

Array:g_modelindex_grand_human, Array:g_modelindex_delux_human, Array:g_modelindex_cr_human,
Array:g_modelindex_bog_human, Array:g_modelindex_coldun_human, Array:g_modelindex_creator_human,

Array:model_grenade_fire_p[64], model_grenade_frost_p[64], model_grenade_infect_p[64],
model_grenade_fire_w[64], model_grenade_frost_w[64], model_grenade_infect_w[64],
...
public plugin_natives()
{
	...
	register_native("menu_zp", "menu_zp", 1)
}
...
public plugin_precache()
{
	...
	model_grand_human = ArrayCreate(32, 1)
	model_delux_human = ArrayCreate(32, 1)
	model_cr_human = ArrayCreate(32, 1)
	model_bog_human = ArrayCreate(32, 1)
	model_coldun_human = ArrayCreate(32, 1)
	model_creator_human = ArrayCreate(32, 1)
	...
	g_modelindex_grand_human = ArrayCreate(1, 1)
	g_modelindex_delux_human = ArrayCreate(1, 1)
	g_modelindex_cr_human = ArrayCreate(1, 1)
	g_modelindex_bog_human = ArrayCreate(1, 1)
	g_modelindex_coldun_human = ArrayCreate(1, 1)
	g_modelindex_creator_human = ArrayCreate(1, 1)
	...
	engfunc(EngFunc_PrecacheGeneric, CLIENT_CONNECT_SOUND)
	// Allow registering stuff now
	g_arrays_created = true
	
	g_fire_exp = precache_model("sprites/BuB9neC/grena/ef_fire_exp.spr")
	g_fire_gibs = precache_model("sprites/BuB9neC/grena/fire_gib.spr")
	
	g_frost_exp = precache_model("sprites/BuB9neC/grena/frost_explode.spr")
	g_frost_gibs = precache_model("sprites/BuB9neC/grena/frost_gib.spr")
	
	g_inf_exp = precache_model("sprites/BuB9neC/grena/infect_exp.spr")
	g_inf_gibs = precache_model("sprites/BuB9neC/grena/infected_gib.spr")
	...
	for (i = 0; i < ArraySize(model_grand_human); i++)
	{
		ArrayGetString(model_grand_human, i, buffer, charsmax(buffer))
		format(buffer, charsmax(buffer), "models/player/%s/%s.mdl", buffer, buffer)
		ArrayPushCell(g_modelindex_grand_human, engfunc(EngFunc_PrecacheModel, buffer))
		if (g_force_consistency == 1) force_unmodified(force_model_samebounds, {0,0,0}, {0,0,0}, buffer)
		if (g_force_consistency == 2) force_unmodified(force_exactfile, {0,0,0}, {0,0,0}, buffer)
	}
	for (i = 0; i < ArraySize(model_delux_human); i++)
	{
		ArrayGetString(model_delux_human, i, buffer, charsmax(buffer))
		format(buffer, charsmax(buffer), "models/player/%s/%s.mdl", buffer, buffer)
		ArrayPushCell(g_modelindex_delux_human, engfunc(EngFunc_PrecacheModel, buffer))
		if (g_force_consistency == 1) force_unmodified(force_model_samebounds, {0,0,0}, {0,0,0}, buffer)
		if (g_force_consistency == 2) force_unmodified(force_exactfile, {0,0,0}, {0,0,0}, buffer)
	}
	for (i = 0; i < ArraySize(model_cr_human); i++)
	{
		ArrayGetString(model_cr_human, i, buffer, charsmax(buffer))
		format(buffer, charsmax(buffer), "models/player/%s/%s.mdl", buffer, buffer)
		ArrayPushCell(g_modelindex_cr_human, engfunc(EngFunc_PrecacheModel, buffer))
		if (g_force_consistency == 1) force_unmodified(force_model_samebounds, {0,0,0}, {0,0,0}, buffer)
		if (g_force_consistency == 2) force_unmodified(force_exactfile, {0,0,0}, {0,0,0}, buffer)
	}
	for (i = 0; i < ArraySize(model_bog_human); i++)
	{
		ArrayGetString(model_bog_human, i, buffer, charsmax(buffer))
		format(buffer, charsmax(buffer), "models/player/%s/%s.mdl", buffer, buffer)
		ArrayPushCell(g_modelindex_bog_human, engfunc(EngFunc_PrecacheModel, buffer))
		if (g_force_consistency == 1) force_unmodified(force_model_samebounds, {0,0,0}, {0,0,0}, buffer)
		if (g_force_consistency == 2) force_unmodified(force_exactfile, {0,0,0}, {0,0,0}, buffer)
	}
	for (i = 0; i < ArraySize(model_coldun_human); i++)
	{
		ArrayGetString(model_coldun_human, i, buffer, charsmax(buffer))
		format(buffer, charsmax(buffer), "models/player/%s/%s.mdl", buffer, buffer)
		ArrayPushCell(g_modelindex_coldun_human, engfunc(EngFunc_PrecacheModel, buffer))
		if (g_force_consistency == 1) force_unmodified(force_model_samebounds, {0,0,0}, {0,0,0}, buffer)
		if (g_force_consistency == 2) force_unmodified(force_exactfile, {0,0,0}, {0,0,0}, buffer)
	}
	for (i = 0; i < ArraySize(model_creator_human); i++)
	{
		ArrayGetString(model_creator_human, i, buffer, charsmax(buffer))
		format(buffer, charsmax(buffer), "models/player/%s/%s.mdl", buffer, buffer)
		ArrayPushCell(g_modelindex_creator_human, engfunc(EngFunc_PrecacheModel, buffer))
		if (g_force_consistency == 1) force_unmodified(force_model_samebounds, {0,0,0}, {0,0,0}, buffer)
		if (g_force_consistency == 2) force_unmodified(force_exactfile, {0,0,0}, {0,0,0}, buffer)
	}
...
public plugin_init()
{
	...
	// Format mod name
	formatex(g_modname, charsmax(g_modname), "[%L]", LANG_SERVER, "GAMENAME")
...
public fw_PlayerSpawn_Post(id)
{
	...
	if (g_handle_models_on_separate_ent)
	{
		...
		{
			iRand = random_num(0, ArraySize(model_creator_human) - 1)
			ArrayGetString(model_creator_human, iRand, g_playermodel[id], charsmax(g_playermodel[]))
			if (g_set_modelindex_offset) fm_cs_set_user_model_index(id, ArrayGetCell(g_modelindex_creator_human, iRand))
		}
		else if (get_user_flags(id) & ADMIN_LEVEL_A)
		{
			iRand = random_num(0, ArraySize(model_coldun_human) - 1)
			ArrayGetString(model_coldun_human, iRand, g_playermodel[id], charsmax(g_playermodel[]))
			if (g_set_modelindex_offset) fm_cs_set_user_model_index(id, ArrayGetCell(g_modelindex_coldun_human, iRand))
		}
		else if (get_user_flags(id) & ADMIN_BAN)
		{
			iRand = random_num(0, ArraySize(model_bog_human) - 1)
			ArrayGetString(model_bog_human, iRand, g_playermodel[id], charsmax(g_playermodel[]))
			if (g_set_modelindex_offset) fm_cs_set_user_model_index(id, ArrayGetCell(g_modelindex_bog_human, iRand))
		}
		else if (get_user_flags(id) & ADMIN_LEVEL_D)
		{
			iRand = random_num(0, ArraySize(model_cr_human) - 1)
			ArrayGetString(model_cr_human, iRand, g_playermodel[id], charsmax(g_playermodel[]))
			if (g_set_modelindex_offset) fm_cs_set_user_model_index(id, ArrayGetCell(g_modelindex_cr_human, iRand))
		}
		else if (get_user_flags(id) & ADMIN_LEVEL_E)
		{
			iRand = random_num(0, ArraySize(model_delux_human) - 1)
			ArrayGetString(model_delux_human, iRand, g_playermodel[id], charsmax(g_playermodel[]))
			if (g_set_modelindex_offset) fm_cs_set_user_model_index(id, ArrayGetCell(g_modelindex_delux_human, iRand))
		}
		else if (get_user_flags(id) & ADMIN_LEVEL_G)
		{
			iRand = random_num(0, ArraySize(model_grand_human) - 1)
			ArrayGetString(model_grand_human, iRand, g_playermodel[id], charsmax(g_playermodel[]))
			if (g_set_modelindex_offset) fm_cs_set_user_model_index(id, ArrayGetCell(g_modelindex_grand_human, iRand))
		}
	...
	else
	{
		...
		if (get_user_flags(id) & ADMIN_KICK)
		{
			size = ArraySize(model_creator_human)
			for (i = 0; i < size; i++)
			{
				ArrayGetString(model_creator_human, i, tempmodel, charsmax(tempmodel))
		...
		{
			size = ArraySize(model_coldun_human)
			for (i = 0; i < size; i++)
			{
				ArrayGetString(model_coldun_human, i, tempmodel, charsmax(tempmodel))
				if (equal(currentmodel, tempmodel)) already_has_model = true
			}
		...
		{
			size = ArraySize(model_bog_human)
			for (i = 0; i < size; i++)
			{
				ArrayGetString(model_bog_human, i, tempmodel, charsmax(tempmodel))
				if (equal(currentmodel, tempmodel)) already_has_model = true
			}
		...
		{
			size = ArraySize(model_cr_human)
			for (i = 0; i < size; i++)
			{
				ArrayGetString(model_cr_human, i, tempmodel, charsmax(tempmodel))
				if (equal(currentmodel, tempmodel)) already_has_model = true
			}
		...
		{
			size = ArraySize(model_delux_human)
			for (i = 0; i < size; i++)
			{
				ArrayGetString(model_delux_human, i, tempmodel, charsmax(tempmodel))
				if (equal(currentmodel, tempmodel)) already_has_model = true
			}
		...
		{
			size = ArraySize(model_grand_human)
			for (i = 0; i < size; i++)
			{
				ArrayGetString(model_grand_human, i, tempmodel, charsmax(tempmodel))
				if (equal(currentmodel, tempmodel)) already_has_model = true
			}
...
public client_connect( id )
{
	...
	client_cmd(id, "mp3 play %s", CLIENT_CONNECT_SOUND)
}
...
public client_putinserver(id)
{
	...
	if (!is_user_bot(id))
	{
		...
		if(get_user_flags(id) & ADMIN_RCON) formatex(g_priv[id], 31, "Основатель")
		else if(get_user_flags(id) & ADMIN_CFG) formatex(g_priv[id], 31, "Смотритель")
		else if(get_user_flags(id) & ADMIN_CVAR) formatex(g_priv[id], 31, "Дизайнер")
		else if(get_user_flags(id) & ADMIN_KICK) formatex(g_priv[id], 31, "Создатель")
		else if(get_user_flags(id) & ADMIN_LEVEL_A) formatex(g_priv[id], 31, "Колднун")
		else if(get_user_flags(id) & ADMIN_BAN) formatex(g_priv[id], 31, "Бог")
		else if(get_user_flags(id) & ADMIN_LEVEL_D) formatex(g_priv[id], 31, "Кристал")
		else if(get_user_flags(id) & ADMIN_LEVEL_E) formatex(g_priv[id], 31, "Делюкс")
		else if(get_user_flags(id) & ADMIN_LEVEL_G) formatex(g_priv[id], 31, "Гранд")
...
public clcmd_buyammo(id)
{
	...
	if (g_zombie[id])
	{
		zp_colored_print(id, "^x04[%L]^x01 %L", id, "GRUPPA", id, "CMD_HUMAN_ONLY")
		return PLUGIN_HANDLED;
	}
	...
	if (g_ammopacks[id] < 1)
	{
		zp_colored_print(id, "^x04[%L]^x01 %L", id, "GRUPPA", id, "NOT_ENOUGH_AMMO")
		return PLUGIN_HANDLED;
	}
	...
	zp_colored_print(id, "^x04[%L]^x01 %L", id, "GRUPPA", id, "AMMO_BOUGHT")
...
show_menu_game(id)
{
	...
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "\r[ZM] %L \r[FREE VIP]^n\r[ZM] \wПривилегия:\r [%s]^n\r[ZM] \wГлавное меню - \r[M]^n^n^n", id, "NAME", g_priv[id])
	...
	// 4. Unstuck
	if (g_isalive[id])
		len += formatex(menu[len], charsmax(menu) - len, "\r4.\w %L^n^n", id, "MENU_UNSTUCK")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d4. %L^n^n", id, "MENU_UNSTUCK")
	
	// 5. Help
	len += formatex(menu[len], charsmax(menu) - len, "\r5.\w %L^n", id, "MENU_KABINET")
	
	// 6. Help
	len += formatex(menu[len], charsmax(menu) - len, "\r6.\w %L^n", id, "MENU_RESPAWN")
	
	// 7. Help
	len += formatex(menu[len], charsmax(menu) - len, "\r7.\w %L^n^n", id, "KUPI_PRIV")
	
	// 8. Help
	len += formatex(menu[len], charsmax(menu) - len, "\r8.\w %L^n", id, "MENU_PRIV")
...
show_menu_extras(id)
{
	...
	{
		zp_colored_print(id, "^x04[%L]^x01 %L", id, "GRUPPA", id ,"CMD_NOT_EXTRAS")
		menu_destroy(menuid)
		return;
	}
		...
		switch (key)
		{
			case 0: // Buy Weapons
			{
				...
				if (get_pcvar_num(cvar_buycustom))
				{
					...
					WPN_AUTO_ON = 0
					zp_colored_print(id, "^x04[%L]^x01 %L", id, "GRUPPA", id, "BUY_ENABLED")
			...
			case 2: // Zombie Classes
			{
				...
				if (get_pcvar_num(cvar_zclasses))
					...
					zp_colored_print(id, "^x04[%L]^x01 %L", id, "GRUPPA", id, "CMD_NOT_ZCLASSES")
			}
			case 3: // Unstuck
			{
			...
			case 6: // kabiet
			{
			client_cmd(id, "menu_information")
			}
			case 7: // kabiet
			{
			client_cmd(id, "menu_priv")
			}
		}
		return PLUGIN_HANDLED;
}
...
public show_txt(id)
{
	show_motd(id,"priv.txt", "Информация")
	return PLUGIN_CONTINUE;
}
...
buy_extra_item(id, itemid, ignorecost = 0)
{
	...
	{
		zp_colored_print(id, "^x04[%L]^x01 %L", id, "GRUPPA", id, "CMD_NOT_CANTUSE")
		return;
	}
...
public menu_zclass(id, menuid, item)
{
	...
	zp_colored_print(id, "^x04[%L]^x01 %L: %s", id, "GRUPPA", id, "ZOMBIE_SELECT", name)
	zp_colored_print(id, "^x04[%L]^x01 %L: %d %L: %d %L: %d %L: %d%%", id, "GRUPPA", id, "ZOMBIE_ATTRIB1", ArrayGetCell(g_zclass_hp, g_zombieclassnext[id]), id, "ZOMBIE_ATTRIB2", ArrayGetCell(g_zclass_spd, g_zombieclassnext[id]),
...
public menu_admin(id, key)
{
	...
	switch (key)
	{
		case ACTION_ZOMBIEFY_HUMANIZE: // Zombiefy/Humanize command
		{
			...
			{
				zp_colored_print(id, "^x04[%L]^x01 %L", id, "GRUPPA", id, "CMD_NOT_ACCESS")
				show_menu_admin(id)
			}
		}
		case ACTION_MAKE_NEMESIS: // Nemesis command
		...
		switch (PL_ACTION)
		{
			...
			case ACTION_MAKE_SURVIVOR: // Survivor command
			{
				if (g_newround ? (userflags & g_access_flag[ACCESS_MODE_SURVIVOR]) : (userflags & g_access_flag[ACCESS_MAKE_SURVIVOR]))
				{
					...
					zp_colored_print(id, "^x04[%L]^x01 %L", id, id, "GRUPPA", "CMD_NOT_ACCESS")
			}
			case ACTION_RESPAWN_PLAYER: // Respawn command
			{
				if (userflags & g_access_flag[ACCESS_RESPAWN_PLAYERS])
...
public cmd_zombie(id, level, cid)
{
	...
	{
		client_print(id, print_console, "[%L] %L", id, "GRUPPA", id, "CMD_NOT")
		return PLUGIN_HANDLED
	}
			...
			case SECTION_PLAYER_MODELS:
			{
				...
				else if (equal(key, "GRAND HUMAN"))
				{
					...
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						// Add to models array
						ArrayPushString(model_grand_human, key)
					}
				}
				else if (equal(key, "DELUX HUMAN"))
				{
					// Parse models
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						// Add to models array
						ArrayPushString(model_delux_human, key)
					}
				}
				else if (equal(key, "CRYSTAL HUMAN"))
				{
					// Parse models
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						// Add to models array
						ArrayPushString(model_cr_human, key)
					}
				}
				else if (equal(key, "BOG HUMAN"))
				{
					// Parse models
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						// Add to models array
						ArrayPushString(model_bog_human, key)
					}
				}
				else if (equal(key, "COLDUN HUMAN"))
				{
					// Parse models
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						// Add to models array
						ArrayPushString(model_coldun_human, key)
					}
				}
				else if (equal(key, "CREATOR HUMAN"))
				{
					// Parse models
					while (value[0] != 0 && strtok(value, key, charsmax(key), value, charsmax(value), ','))
					{
						...
						// Add to models array
						ArrayPushString(model_creator_human, key)
					}
				}
...
public welcome_msg()
{
	...
	zp_colored_print(0, "^x04[%L]^x01 %L", LANG_PLAYER, "GRUPPA", LANG_PLAYER, "NOTICE_INFO1")
...
check_round(leaving_player)
{
	...
	if (g_zombie[leaving_player] && fnGetZombies() == 1)
	{
		...
		zp_colored_print(0, "^x04[%L]^x01 %L", id, "GRUPPA", LANG_PLAYER, "LAST_ZOMBIE_LEFT", g_playername[id])
	...
	else if (!g_zombie[leaving_player] && fnGetHumans() == 1)
	{
		...
		zp_colored_print(0, "^x04[%L]^x01 %L", id, "GRUPPA", LANG_PLAYER, "LAST_HUMAN_LEFT", g_playername[id])
...
public ShowHUD(taskid)
{
	...
	if (id != ID_SHOWHUD)
	{
		...
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "%L %s^nHP: %d - %L %s - %L %d | Игрушки: %d", ID_SHOWHUD, "SPECTATING", g_playername[id], pev(id, pev_health), ID_SHOWHUD, "CLASS_CLASS", class, ID_SHOWHUD, "AMMO_PACKS1", g_ammopacks[id], sp_get_toy(id))
	}
	//else000000000
	...
	if (g_zombie[id] )
	show_hudmessage(ID_SHOWHUD, "%L: %d | %L %s^n%L %d | Игрушки: %d", id, "ZOMBIE_ATTRIB1", pev(ID_SHOWHUD, pev_health), ID_SHOWHUD, "CLASS_CLASS", class, ID_SHOWHUD, "AMMO_PACKS1", g_ammopacks[ID_SHOWHUD], sp_get_toy(id))
	else
	show_hudmessage(ID_SHOWHUD, "%L %s | %L %d^n%L: %d | %L: %d^nИгрушки: %d", id, "CLASS_CLASS", class, ID_SHOWHUD, "AMMO_PACKS1", g_ammopacks[ID_SHOWHUD], ID_SHOWHUD, "ZOMBIE_ATTRIB1", pev(ID_SHOWHUD, pev_health), ID_SHOWHUD, "ZOMBIE_ATTRIB5", get_user_armor(id), sp_get_toy(id))
}
//}
...
public reklama()
{
	for(new id = 1; id <= 32; id++)
	{
		...
		if(g_zombie[id])
		set_dhudmessage(0, 255, 255, 0.02, 0.21, 0, 6.0, 1.1, 0.0, 0.0);
		else
		set_dhudmessage(0, 255, 255, 0.02, 0.21, 0, 6.0, 1.1, 0.0, 0.0);
		show_dhudmessage(id, "Владелец: %L^nГруппа [В]к: %L", id, "SITE", id, "GRUPPA")
	}
}
...
command_zombie(id, player)
{
	ColorChat(0, "!yАдминистратор !g%s !yсделал !g%s Зомби!", g_playername[id], g_playername[player])
...
command_human(id, player)
{
	ColorChat(0, "!yАдминистратор !g%s !yсделал !g%s Человеком!", g_playername[id], g_playername[player])	
...
command_survivor(id, player)
{
	ColorChat(0, "!yАдминистратор !g%s !yсделал !g%s Соником!", g_playername[id], g_playername[player])	
...
command_nemesis(id, player)
{
	ColorChat(0, "!yАдминистратор !g%s !yсделал !g%s Электроником!", g_playername[id], g_playername[player])
...
command_respawn(id, player)
{
	ColorChat(0, "!yАдминистратор !g%s !yВозродил !g%s!", g_playername[id], g_playername[player])
...
command_swarm(id)
{
	ColorChat(0, "!yАдминистратор !g%s !yсделал !gКучу На Кучу!", g_playername[id])
...
command_multi(id)
{
	ColorChat(0, "!yАдминистратор !g%s !yсделал !gМассовое Заражение!", g_playername[id])
...
command_plague(id)
{
	ColorChat(0, "!yАдминистратор !g%s !yсделал !gЧуму!", g_playername[id])
